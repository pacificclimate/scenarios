package CICS::Scenario::Displayer;
use strict;
...
sub make_data_row_header {
  ...
  $text .= "<tr><th>" . $title . "</th><th>GIS</th><th>Georef</th><th>Timeseries (".$unit.")</th><th>Timeseries (" . $unit . ") anomaly</th></tr>";
...
sub make_data_row {
  my($self, $desc, $rowdesc) = @_;
  my(@items, $ftype);
  my(@datafmts) = ([ $self->{ss}->{'geotiff_format'}, TYPE_GEOTIFF ], [ $self->{ss}->{'georeferenced'}, TYPE_GEOREF ]);
  my($datafilename);
  my($modelname) = $self->{exptdata}->[$desc->{'expt'}]->{'modelname'};
...
sub make_region_img {
  ...
  $output .= "<input type=\"image\" name=\"regionmap\" class=\"crosshaircursor\" src=\"".$wrapper."?".mkgetstring($desc)."\" alt=\"Reload to view region\" style=\"width: ";
