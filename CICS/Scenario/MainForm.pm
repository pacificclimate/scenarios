package CICS::Scenario::MainForm;
use strict;
...
use constant {
  OP_ADD => 1, OP_SELECT => 2, OP_MOVE => 3, OP_ZOOMIN => 4
};

use constant {
  DESELECT_PT => 0, REMOVE_PT => 1, CLEAR_PTS => 3, ZOOMOUT => 4
};
...
sub new {
  ...
  if(defined($self->{post}->{planners})) {  #FIXME magic numbers.
      $self->{ts}[0] = "";
      $self->{ts}[4] = "";
  }
  ...
  # Update button
  $form->addBit(CICS::FormHandler::Submitbutton->new({name => 'update', cssclass => "submit", allowed_values => [ $self->{ss}->{update} ]}));
  ...
  # FIXME maybe move this somewhere less gross
  my($temp_pd) = CICS::Scenario::Helpers::parse_textdata($self->{cache}->create_cachefile($self->get_first_desc($hash), TYPE_TEXT));
  # x (lon) and y (lat) center coordinates for zoomed window centre, if different from window centre  FIXME allowed_range not implemented yet!!
  if(!defined($self->{post}->{view_x}) || !defined($self->{post}->{view_y})) {
      ($self->{post}->{view_x}, $self->{post}->{view_y}) = (($temp_pd->{mapminlon} + $temp_pd->{mapmaxlon} / 2), ($temp_pd->{mapminlat} + $temp_pd->{mapmaxlat}) / 2);
      ($hash->{view_x}, $hash->{view_y}) = ($self->{post}->{view_x}, $self->{post}->{view_y});
  }
  ...
  if(($hash->{pr} != 0 || $hash->{oldpr} != 0) && ($hash->{pr} != $#{$self->{regions}} || $hash->{oldpr} != $#{$self->{regions}})) {
    $self->{post}->{points} = $self->{regions}->[$hash->{pr}]->{coords};
    $self->{post}->{th} = $self->{regions}->[$hash->{pr}]->{threshold};
    if(defined($self->{post}->{planners}) && !$wrapper) { # FIXME take out the 1 once planners frontend can be used for testing images.
      if ($hash->{pr} == 0) { # we have CHANGED TO no region, so clear zoom and view_[xy] -- this relies on populate being called later again.
        ...
        delete $self->{post}->{'zoom'};
      } elsif ($hash->{pr} != $#{$self->{regions}}) { # we have changed to a predefined (as opposed to custom) region, so autozoom.
	my($windowwidth, $windowheight) = ($temp_pd->{'mapmaxlon'} - $temp_pd->{'mapminlon'}, $temp_pd->{'mapmaxlat'} - $temp_pd->{'mapminlat'});

	# Find region bounds
	my($minregionlat, $maxregionlat, $minregionlon, $maxregionlon) = (999999999,-999999999,999999999,-999999999);
	foreach (split (/,/, $self->{post}->{points})) {
	  my($lon, $lat) = split(/:/);
	  if ($lat < $minregionlat) { $minregionlat = $lat; }
	  if ($lat > $maxregionlat) { $maxregionlat = $lat; }
	  if ($lon < $minregionlon) { $minregionlon = $lon; }
	  if ($lon > $maxregionlon) { $maxregionlon = $lon; }
	}

	# Set center
	$self->{post}->{'view_x'} = ($minregionlon + $maxregionlon) / 2;
	$self->{post}->{'view_y'} = ($minregionlat + $maxregionlat) / 2;
      
	# Determine zoom level
	my($widthratio, $heightratio) = (($maxregionlon - $minregionlon) / $windowwidth, ($maxregionlat - $minregionlat) / $windowheight);
...
sub handle_buttons {
  ...
  if($bop eq $bops->[DESELECT_PT]) {
    ...
    $regionchange = 1;
  } elsif($bop eq $bops->[ZOOMOUT] && $hash->{zoom} > 0) {
      $self->{post}->{zoom} = $hash->{zoom} - 1;
  }
...
sub get_click_coords {
  ...
  # Translate view_x and view_y to coordinate system if needed
  my($view_x, $view_y) = ($hash->{view_x}, $hash->{view_y});
  if(defined($pd->{proj4_string}) && $pd->{proj4_string} ne "") {
      my($proj4) = Geo::Proj4->new($pd->{proj4_string}) or die "parameter error: ".Geo::Proj4->error. "\n";
      ($view_x, $view_y) = $proj4->forward($hash->{view_y}, $hash->{view_x});
      if(!defined($view_x)) {
	  print STDERR "Zoom PROJECTION FAILED\n";
      }
  }

  my($size_x_2) = ($pd->{mapmaxlon} - $pd->{mapminlon}) / $self->{d}->{zoom}->[$hash->{zoom}] / 2;
  my($size_y_2) = ($pd->{mapmaxlat} - $pd->{mapminlat}) / $self->{d}->{zoom}->[$hash->{zoom}] / 2;
  my($center_x, $center_y);
  if($hash->{region} == 0) { # World...
      $center_x = $view_x;
  } else {
      $center_x = clip_to_range($pd->{mapminlon} + $size_x_2, $pd->{mapmaxlon} - $size_x_2, $view_x);
  }
  $center_y = clip_to_range($pd->{mapminlat} + $size_y_2, $pd->{mapmaxlat} - $size_y_2, $view_y);
  my($zoomminlat, $zoomminlon, $zoommaxlat, $zoommaxlon) = ($center_y - $size_y_2, $center_x - $size_x_2, $center_y + $size_y_2, $center_x + $size_x_2);

  print STDERR "Zoom view center: " . join(",", $view_x, $view_y) . "\n";
  print STDERR "Zoom corrected center: " . join(",", $center_x, $center_y) . "\n";
  print STDERR "Zoom map win: " . join(",", $pd->{mapminlat}, $pd->{mapminlon}, $pd->{mapmaxlat}, $pd->{mapmaxlon}) . "\n";
  print STDERR "Zoom win: " . join(",", $zoomminlat, $zoomminlon, $zoommaxlat, $zoommaxlon) . "\n";

  # Translate latitudes and longitudes
  if(!defined($pd->{proj4_string}) || $pd->{proj4_string} eq "") {
      $lon = xtolon($zoomminlon, $zoommaxlon, $pd->{offset_x}, $pd->{mapsize_x}, $hash->{'map.x'});
      $lat = ytolat($zoomminlat, $zoommaxlat, $pd->{offset_y}, $pd->{mapsize_y}, $hash->{'map.y'});
  } else {
      my($proj4) = Geo::Proj4->new($pd->{proj4_string}) or die "parameter error: ".Geo::Proj4->error. "\n";
      my($xc) = xtolon($zoomminlon, $zoommaxlon, $pd->{offset_x}, $pd->{mapsize_x}, $hash->{'map.x'});
      my($yc) = ytolat($zoomminlat, $zoommaxlat, $pd->{offset_y}, $pd->{mapsize_y}, $hash->{'map.y'});
...
sub handle_regionmapclick {
  ...
  if(defined($lon) && defined($lat)) {
	...
	$points->[$i->[0]] = $point;
      ...
      $regionchange = 1;
    } elsif($hash->{op} == OP_ZOOMIN) {
	$self->{post}->{view_x} = $lon;
	$self->{post}->{view_y} = $lat;
	$self->{post}->{zoom} = ($hash->{zoom} < $#{$self->{dd}->{zoom}} ? $hash->{zoom} + 1 : $hash->{zoom});
    }
...
sub process_ts {
  ...
  for($i = 0; $i <= $numi; $i++) {
	...
	if(is_ts_absolute($list->[$i], $hash->{expt}, $self->{exptdata})) {
	    ($hash->{r_min}, $hash->{r_max}) = @{$self->{dat}->[5]->{variable}->[$hash->{var}]};
	} else {
	    ($hash->{r_min}, $hash->{r_max}) = @{$self->{dat}->[6]->{variable}->[$hash->{var}]};
	}
...
sub make_desc_list {
  ...
  my($tslist) = $self->{"exptdata"}[$hash->{"expt"}]{"netcdf_periods"}[$hash->{ts}];
