...
function crossDomainXMLHttpRequest(base_url, params, handler) {
    var url = OpenLayers.Util.urlAppend(base_url,
...
function init_map(map_id, region, climate_overlay, climate_time, color_range, color_scale, center_point, zoom_level) {
    ...
    ncwms =  new OpenLayers.Layer.WMS("ncwmsLayer",
                                      "http://medusa.pcic.uvic.ca/ncWMS/wms?",
                                      params,
                                      {buffer: 1, ratio: 1.5, 
	...
	"OpenStreetMap", 
	"http://medusa.pcic.uvic.ca/geoserver/gwc/service/wms",
    ...
    {
        if (url == null) url="http://medusa.pcic.uvic.ca/geoserver/p2a/wms";
    ...
    addWMSLayer("Glaciers", 'cwb_glaciers_gwc', "http://medusa.pcic.uvic.ca/geoserver/gwc/service/wms", null, null, null, null, true,  map.maxExtent.left + ',' + map.maxExtent.bottom);
    ...
    addWMSLayer("Land Cover", 'btmv1_landuse_gwc', "http://medusa.pcic.uvic.ca/geoserver/gwc/service/wms", null, null, null, null, true,  map.maxExtent.left + ',' + map.maxExtent.bottom);
    addWMSLayer("Biogeoclimatic Zones", 'bec_zones_gwc', "http://medusa.pcic.uvic.ca/geoserver/gwc/service/wms", null, null, null, null, true,  map.maxExtent.left + ',' + map.maxExtent.bottom);
    ...
    // bring in the opacity slider control
    map.addControl(new OpenLayers.Control.OpacitySlider({layerToOpacisize:ncwms}));
...
function getFeatureInfo(e)
{
    ...
    {
        var wmsurl = "http://medusa.pcic.uvic.ca/geoserver/p2a/wms?";
        var type = 'html'
        var url = wmsurl
            + "REQUEST=GetFeatureInfo"
            + "&EXCEPTIONS=application/vnd.ogc.se_xml"
