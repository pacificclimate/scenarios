...
function init_map(map_id, analysis_region, region, climate_overlay, climate_time, color_range, color_scale, center_point, zoom_level) {
    var P4326 = new OpenLayers.Projection("EPSG:4326");
    var P3005 = new OpenLayers.Projection("EPSG:3005");
    ...
    var options = {
        ...
        maxResolution: 4437,
        resolutions: [4437, 2218.5, 1109.25, 554.625, 277.3125],
    ...
    var analysisRegionLookup = {
	'Regional Districts': 'p2a:rd_with_bbox',
	'Health Authority Boundaries': 'p2a:had_with_bbox',
	'Ecoprovinces': 'p2a:ecoprovince_with_bbox',
	'Forestry Regions': 'p2a:nro_reg_with_bbox'
    };
...
//    munilink = '<a href=\"https://apps.gov.bc.ca/pub/geometadata/metadataDetail.do?recordUID=50339&recordSet=ISO19115\">?</a>'
	...
	// only add a greyed out analysis region if it is not the BC polygon
	addWMSLayer(analysis_region, analysisRegionLookup[analysis_region], null, false, filter_region_xml, true, 0.43);
...
function getWMSrequest(topLayer, currentMap, e_xy)
{
...
function addNcwmsPopup(trimmedName, currentMap, url, lonLat, minRange, maxRange, closeBoxCallback)
{
...
function getFeatureInfo(currentMap, location, e_xy, closeBoxCallback, minRange, maxRange)
{
    ...
    var trimmedName = topLayer.params.LAYERS.replace('_gwc', '');
    url = getWMSrequest(topLayer, currentMap, e_xy);
    if (topLayer.name == 'ncwmsLayer') {
	addNcwmsPopup(trimmedName, currentMap, url, lonLat, minRange, maxRange, closeBoxCallback);
    } else {
	addLayerPopup(trimmedName, currentMap, url, lonLat, minRange, maxRange, closeBoxCallback, e_xy, minRange, maxRange);
    };
};

function addLayerPopup(trimmedName, currentMap, url, lonLat, minRange, maxRange, closeBoxCallback, e_xy, minRange, MaxRange){
    ...
    OpenLayers.loadURL(url, '', currentMap, function(response){
	    ...
	    topLayer = this.getLayersByName('ncwmsLayer')[0];
	    url = getWMSrequest(topLayer, this, e_xy, lonLat);
            //this.removePopup(tempPopup);
	    var trimmedName = topLayer.params.LAYERS.replace('_gwc', '');
	    addNcwmsPopup(trimmedName, this, url, lonLat, minRange, maxRange, closeBoxCallback);
        };
    });
};
