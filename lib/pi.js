...
function zoomImg(foo) {
	...
	if(foo % 2 == 0) {
		...
		for(i = 0; i < map1.popups.length; i++) {
		    map1.removePopup(map1.popups[i]);
		}
		for(i = 0; i < map2.popups.length; i++) {
		    map2.removePopup(map2.popups[i]);
		}
	    }
...
function hideTRClass(c) {
    jQuery('.' + c).css('display', 'none');
}

function showTRClass(c) {
    jQuery('.' + c).css('display', 'table-row');
}
