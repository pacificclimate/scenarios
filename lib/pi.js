...
//	document.getElementById(sidetabs[seltab]).className = 'inactive';
        $("#" + sidetabs[seltab]).removeClass('active').addClass('inactive');
	seltab = tab;
//	document.getElementById(sidetabs[seltab]).className = 'active';
        $("#" + sidetabs[seltab]).removeClass('inactive').addClass('active');
...
function zoomImg(foo) {
	...
	if(foo % 2 == 0) {
	    ...
	    var map1 = init_map(past[0], past[1], past[2], past[3], past[4], past[5], past[6], past[7], past[8]);
	    var map2 = init_map(fut[0], fut[1], fut[2], fut[3], fut[4], fut[5], fut[6], fut[7], fut[8]);
	    ...
	    map1.addControl(new OpenLayers.Control.PanZoomBar({position: new OpenLayers.Pixel(2, 15),
							       panIcons:false}));
	    map2.addControl(new OpenLayers.Control.ScaleLine());
		...
		if(!popMap1){
		    ...
		    getFeatureInfo(map2, popupLoc, e.xy, closeBoxCallback, fut[14], fut[15]);
		    popMap1 = false;
		};
	    });
	    ...
	    try {
		draw_raphael_legend(past[9], past[6], past[10], past[11], past[12], past[13]);
	    } catch(e) { alert("fubar")/* silently fail */ };
